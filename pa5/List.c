...
List newList(void){
   List myList = malloc(sizeof(ListObj));
   myList->front = NULL;
   myList->back = NULL;
   myList->length = 0;
...
int getElement(List L){
   ...
   if(L->cruz->index == -1){
      printf("List ERROR: calling getElement() when cruz index == -1\n");
      exit(1);
   }
   if(L->cruz->pointer == NULL){
      printf("In getElement(): cruz->pointer == NULL\n");
   }
...
void moveTo(List L, int i){
   ...
   if(L->front == NULL){
      printf("moveTo() L->front == NULL\n");
   }
   if(L->back == NULL){
      printf("moveTo() L->back == NULL\n");
   }
   L->cruz->index = 0;
   ...
   if(L->cruz->pointer == NULL){
      printf("moveTo() L->cruz->pointer is still null \n");
   }
}
